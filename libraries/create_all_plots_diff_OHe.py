...
def create_all_plots(folders,fig_save_dir, forced_exponent = None, forced_dt=None, t_end=None, t_plot=None, h=0.05, 
    ...
    for f in folders:
        ...
        if l in skip_Oh: 
            continue
        Ohd = float(re.search(r"_Ohe_([^_]+)", f.split("/")[-1]).group(1).replace("p", "."))
        Ohf = l
    ...
    if save_plots:
        ...
        save_plot(444, fig_save_dir+'rvst_collapsed'+str(h)+'.png', dpi=300)
    ...
    for i in range(len(Ohd_list)):
        ...
        if i <4:
            ...
            temp_label = None
        create_plot((np.array(Oh[i])**1), np.array(D_list[i]), 99, label =temp_label, alpha = 0.6,  # 0.9 therm to account for difference paper D0 and our D0 in surface tension
