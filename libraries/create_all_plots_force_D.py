...
def create_all_plots(folders,fig_save_dir, forced_exponent = None, forced_dt=None, t_end=None, t_plot=None, h=0.05, 
            rhof=0.9, plot_individual_plots=False, save_plots=True, linestyle="--", skip_Oh=[], marker=".", forced_D=None):
    """
    ...
    for f in folders:
        ...
        if forced_D is not None:
            aa = np.sqrt(2*forced_D[i])
        a, b, c, pcov = get_fit(t_s, r_s, cc=forced_exponent, bb=bb, aa=aa)
        ...
        create_plot((t_o-dt)[(r_o>0.1)], r_o[(r_o>0.1)], 444, fmt=".", color=color, label=label, xlabel=r'$\tilde t - \tilde t_0$', ylabel=r'${\tilde{r}}$', 
                    title=None, alpha=0.7, xscale="log", yscale="log",  markersize=2)
        create_plot((t_o-dt)[(r_o>0.1)], func(t_o, a, dt, 0.5)[(r_o>0.1)], 444, fmt="--", color=color, label=label, xlabel=r'$(\tilde t - \tilde t_0)$', ylabel=r'${\tilde{r}}$', 
                    title=None, alpha=0.7, xscale="log", yscale="log",  markersize=2)

        plt.xlim(1e-2, 4)
        plt.ylim(1e-1, 2)
        create_plot((t_o-dt)[(r_o>0.1)]*2*D, r_o[(r_o>0.1)], 5, fmt=".", color=color, label=label, xlabel=r'$(\tilde t - \tilde t_0) 2D$', ylabel=r'${\tilde{r}}$', 
                    title=None, alpha=0.7, xscale="log", yscale="log",  markersize=2)
        create_plot((t_o-dt)[(r_o>0.1)]*2*D, func(t_o, a, dt, 0.5)[(r_o>0.1)], 5, fmt="--", color=color, label=label, xlabel=r'$(\tilde t - \tilde t_0) 2D$', ylabel=r'${\tilde{r}}$', 
                    title=None, alpha=0.7, xscale="log", yscale="log",  markersize=2)
    ...
    asymptote_low = 1/np.sqrt(alpha) * np.ones(len(Oh_asymptotes)) *1.3
    ...
    if save_plots:
        data_Nath = np.array([[0.7790891638841551, 0.32696037174989717],
                            ...
                            [202.37546008116516, 0.08529694459231732]]) 
                                 # account for different normalization of surface tension in Nath &quere paper


        
        create_plot(data_Nath[:,0] * np.sqrt(0.7/0.6), data_Nath[:,1] * np.sqrt(0.7/0.6), 99, label ="Nath & Queré", alpha = 0.75,  
