...
def gettingTriplePoint(filename, name1, DistCutoff):
    exe = ["./getX0Y0V0WithBubblesEntrapped", filename, name1, str(DistCutoff)]
    p = sp.Popen(exe, stdout=sp.PIPE, stderr=sp.PIPE)
    stdout, stderr = p.communicate()
    temp1 = stderr.decode("utf-8")
    temp2 = temp1.split("\n")
...
if len(sys.argv) == 5:
    ...
    folder = save_folder + '/TrackingTPWithBubblesEntrapped' # output folder
    intermediate_folder = save_folder + '/intermediate'
elif len(sys.argv) == 4:
    folder = 'TrackingTPWithBubblesEntrapped' # output folder
...
np.savetxt(save_folder +"/tp_dataWithBubblesEntrapped.csv", np.array(tp_list), header='t zTP rTP vTP theta1 theta2')