...
def create_all_plots(folders,fig_save_dir, forced_exponent = None, forced_dt=None, t_end=None, t_plot=None, h=0.05, 
    ...
    # plot D vs Oh
    Ohd_list = np.array(Ohd_list)
    Ohf_list = np.array(Oh)
    ...
    for i in range(len(Ohd_list)):
        ...
        if i <4:
            ...
            temp_label = None
        create_plot((np.array(Oh[i])**1* (R/h)**1), np.array(D_list[i]), 99, label =temp_label, alpha = 0.6,  # 0.9 therm to account for difference paper D0 and our D0 in surface tension
                    fmt=fmt, xscale="log", yscale="log", xlabel=r"$Oh_{film}\cdot R/h$", 
                    ylabel = r"$D/D_0$", markerfacecolor='none', markeredgewidth=1)
        create_plot(np.array(Oh) * (R/h), np.sqrt(np.array(D_list))/(1+Ohf_list*Ohd_list*R/h), 98, label ="h="+str(h), alpha = 0.6,  # 0.9 therm to account for difference paper D0 and our D0 in surface tension
    ...
    if save_plots:
        ...
        #             fmt="k>", xscale="log", yscale="log", xlabel=r"$Oh_{film}\cdot R/h$", 
        #             ylabel = r"$D/D_0$", markeredgewidth=1, markerfacecolor='none')
        # # create_plot(data_Nath[:,0]/(1+data_Nath[:,0]*5e-2), np.sqrt(data_Nath[:,1]), 98, label ="Nath & Queré", alpha = 0.75,  
        # #             fmt="ks", xscale="log", yscale="log", xlabel=r"$Oh_{film}\cdot R/h \cdot V_{factor}$", 
        # #             ylabel = r"$D/D_0$", markeredgewidth=1, markerfacecolor='none')
        ...
        save_plot(98, fig_save_dir+'DvsOh_with_slip_correction.png', dpi=300)
    ...
    if save_plots:
        ...
        print(t_start)
    return Oh, D_list, t_start, h, Ohd_list